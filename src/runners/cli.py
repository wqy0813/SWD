...
from ..core.models import Module, SubBlock
from ..core.visualize import visualize_floorplan
from ..problems.nonrect import NonRectSolver
from ..problems.solver import ProblemSolver
from .artifacts import write_result_summary
from . import run_problem1, run_problem2


def solve_chip(problem_path: str, chip_name: str, problem: int = 0):
    """
    ...
    output_root = os.path.join("outputs", chip_name)

    results = {'chip': chip_name}

    if problem in (0, 1):
        results['problem1'] = run_problem1.run(solver, chip_name, output_root)

    if problem in (0, 2):
        results['problem2'] = run_problem2.run(
            solver,
            chip_name,
            output_root,
    ...
    if problem in (0, 3):
        ...
        problem3_dir = os.path.join(output_root, "problem3")
        os.makedirs(problem3_dir, exist_ok=True)
        problem3_summary = os.path.join(
            problem3_dir,
            f"{chip_name}_problem3_summary.txt",
        )
        visualize_floorplan(result3, solver.hard_modules,
                            title=f"{chip_name} - Problem 3: Min DSR = {min_dsr:.4f}",
                            save_path=os.path.join(problem3_dir, f"{chip_name}_problem3_floorplan.png"),
                            show_nets=True,
                            nets=solver.nets,
                            terminal_positions=solver.terminal_positions)
        write_result_summary(result3, problem3_summary)
        results['problem3'] = result3
        results['min_dsr'] = min_dsr
...
def main():
    ...
    if args.sample:
        ...
        print("=" * 60)
        sample_output_root = os.path.join("outputs", "sample")

        if args.problem in (0, 1):
            run_problem1.run(solver, "sample", sample_output_root)
        ...
        if args.problem in (0, 3):
            min_dsr, result3 = solver.solve_problem3()
            sample_problem3_dir = os.path.join(sample_output_root, "problem3")
            os.makedirs(sample_problem3_dir, exist_ok=True)
            visualize_floorplan(result3, solver.hard_modules,
                                title=f"Sample - Problem 3: Min DSR = {min_dsr:.4f}",
                                save_path=os.path.join(
                                    sample_problem3_dir,
                                    "sample_problem3_floorplan.png",
                                ),
                                show_nets=True,
                                nets=solver.nets,
                                terminal_positions=solver.terminal_positions)
            write_result_summary(
                result3,
                os.path.join(sample_problem3_dir, "sample_problem3_summary.txt"),
            )
        ...
        results = solve_chip(args.path, args.chip, problem=args.problem)
